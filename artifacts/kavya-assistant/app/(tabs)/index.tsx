...
type Stage = 'welcome' | 'permissions' | 'geminiSetup' | 'settings' | 'assistant';
...
// NOTE: SecureStore key names may ONLY contain letters, numbers, "." "-" "_".
// The old prefix "@kavya/gemini-key/" used "@" and "/" which SecureStore
// silently rejects — that is why keys were not saving before. Fixed below.
const GEMINI_KEY_PREFIX = 'kavya_gemini_key_';

// TEMP DEFAULT KEY — you asked to hardcode this for now so Gemini works out
// of the box. This is only meant as a placeholder: anyone who has the built
// APK (or the git history) can extract this key, so please swap it for your
// own key and delete this constant once you're done testing.
const DEFAULT_GEMINI_API_KEY = 'AQ.Ab8RN6JZMazVaSqdD2xTtbMGeISZRzvGku_kmFTLE5vh-Ur4-Q';

const GEMINI_MODEL = 'gemini-2.0-flash';

const capabilitiesList = [
  'Time aur aaj ki date batana',
  'Timer aur alarm set karna',
  'Google Maps par jagah dhoondhna (hospital, restaurant, etc.)',
  'YouTube par kuch search karke kholna',
  'WhatsApp message ya SMS draft ready karna',
  'Number dial karna (dialer kholna)',
  'Gemini se general sawaalon ke smart jawab dena, bolkar bhi',
];
...
// Speak a reply out loud so the conversation feels two-way, not just text.
function speak(text: string) {
  if (Platform.OS === 'web' || !text) return;
  try {
    Speech.stop();
    Speech.speak(text, { language: 'hi-IN', pitch: 1.0, rate: 1.0 });
  } catch {
    // If TTS isn't available on this device we just fall back to text-only reply.
  }
}
  ...
  const response = await fetch(
    `https://generativelanguage.googleapis.com/v1beta/models/${GEMINI_MODEL}:generateContent?key=${apiKey}`,
    {
      ...
      body: JSON.stringify({
        ...
        systemInstruction: {
          parts: [{ text: 'Tum Kavya ho, ek friendly Hindi-English voice assistant jo phone par chalti hai. Chhote, natural, bolne layak jawab do.' }],
        },
      }),
    }
  );
...
export default function AssistantScreen() {
  ...
  // Sets the on-screen reply AND speaks it, so voice replies feel like a real conversation.
  ...
  const saveGeminiKey = async () => {
    ...
    try {
      ...
      await persistKeyIndex([...savedKeys, meta], id);
      setNewGeminiKey(''); setNewGeminiLabel('');
      Alert.alert('Key securely saved', `${meta.label} backup list mein save ho gayi.`);
    } catch (error) {
      Alert.alert('Save nahi ho payi', 'Key device storage mein save karte waqt error aaya. Dobara try karo.');
    }
  };

  const chooseGeminiKey = async (id: string) => {
    try { await persistKeyIndex(savedKeys, id); }
    catch { Alert.alert('Switch nahi ho paya', 'Active key badalte waqt error aaya.'); }
  };
  ...
  const removeGeminiKey = async (id: string) => {
    ...
    } catch { Alert.alert('Delete nahi hui', 'Key delete karte waqt error aaya.'); }
  };

  // Returns the key to actually call Gemini with: the user's active saved key
  // if there is one, otherwise the built-in default key.
  const resolveApiKey = async (): Promise<string> => {
    const id = activeKeyIdRef.current;
    if (id) {
      try {
        const stored = await SecureStore.getItemAsync(`${GEMINI_KEY_PREFIX}${id}`);
  ...
  const runCommand = useCallback(async (input?: string) => {
    ...
    setListening(false);
    if (normalized.includes('time')) say(`Abhi ${new Date().toLocaleTimeString([], { hour: 'numeric', minute: '2-digit' })} baj rahe hain.`);
    else if (normalized.includes('date') || normalized.includes('today')) say(`Aaj ${new Date().toLocaleDateString('en-IN', { weekday: 'long', day: 'numeric', month: 'long' })} hai.`);
      ...
      setTimerSeconds(seconds); say(`Timer ${minuteMatch ? `${Number(minuteMatch[1])} minute` : `${seconds} second`} ke liye set hai.`);
      setTimeout(() => { setTimerSeconds(null); say('Timer complete ho gaya.'); }, seconds * 1000);
    } else if (normalized.includes('alarm')) {
      const timeMatch = normalized.match(/(\d{1,2})(?::(\d{2}))?\s*(am|pm)?/);
      if (!timeMatch) say('Alarm ka time bolo, jaise “alarm 7:30 AM”.');
      else {
        ...
        say('Alarm ready hai. Confirm karne ke baad Clock khulega.');
      }
    } else if (normalized.includes('youtube') || normalized.includes('music')) {
      setPendingAction({ title: 'YouTube search kholni hai?', detail: original, uri: `https://www.youtube.com/results?search_query=${encodeURIComponent(original)}` }); say('YouTube search ready hai. Confirm kar do.');
    } else if (normalized.includes('map') || normalized.includes('hospital') || normalized.includes('restaurant')) {
      setPendingAction({ title: 'Google Maps search kholni hai?', detail: original, uri: `https://www.google.com/maps/search/${encodeURIComponent(original)}` }); say('Maps search ready hai. Confirm kar do.');
      ...
      setPendingAction({ title: 'WhatsApp message ready hai', detail: body || 'Message text missing hai.', uri: `whatsapp://send?text=${encodeURIComponent(body)}` }); say('WhatsApp kholne se pehle tumhari confirmation chahiye.');
    } else if (normalized.includes('call')) {
      const number = original.match(/\+?\d[\d\s-]{7,}/)?.[0]?.replace(/[^\d+]/g, '');
      if (!number) say('Call ke liye number bolo, jaise “call 9876543210”.');
      else { setPendingAction({ title: 'Dialer kholna hai?', detail: number, uri: `tel:${number}` }); say('Number mil gaya. Confirm karne ke baad dialer khulega.'); }
      ...
      if (!number) say('SMS ke liye number bolo, jaise “SMS 9876543210 saying hello”.');
      else { setPendingAction({ title: 'SMS draft kholna hai?', detail: `${number}${body ? `\n${body}` : '\n(Message text missing hai)'}`, uri: `sms:${number}${body ? `?body=${encodeURIComponent(body)}` : ''}` }); say('SMS draft ready hai. Send karne se pehle phone mein check kar lena.'); }
    } else if (normalized.includes('kya kar sakti') || normalized.includes('kya kar sakte') || normalized.includes('what can you do') || normalized.includes('capabilities') || normalized === 'help') {
      say(`Main ye sab kar sakti hoon: ${capabilitiesList.join(', ')}. Har action se pehle main tumse confirm karti hoon.`);
    } else {
      setThinking(true);
      say('Soch rahi hoon...');
      try {
        const apiKey = await resolveApiKey();
        const answer = await askGemini(apiKey, original);
        say(answer);
      } catch (error) {
        say('Gemini se abhi jawab nahi mil paya. Internet ya API key check karo.');
      } finally {
        setThinking(false);
      }
    }
  ...
  const confirmAction = async () => {
    ...
    try { await Linking.openURL(pendingAction.uri); say(`${pendingAction.title.replace('?', '')} complete.`); }
    catch { say('Ye action is phone par available nahi hai.'); }
  ...
  if (stage === 'permissions') {
    return (
      <ScrollView style={styles.screen} contentContainerStyle={{ paddingTop: insets.top + 24, paddingBottom: insets.bottom + 28 }} showsVerticalScrollIndicator={false}>
        <View style={styles.brandRow}><Pressable onPress={() => setStage('welcome')}><Feather name="arrow-left" size={22} color={colors.foreground} /></Pressable><Text style={styles.pageTitle}>Phone access</Text><Text style={styles.step}>1 / 2</Text></View>
        <Text style={styles.sectionTitle}>Kavya ko aapki awaaz sunni hai</Text>
        <Text style={styles.sectionBody}>Sirf wahi permissions allow karo jo tum use karna chahte ho. Agle step mein Gemini key add karne ka option milega.</Text>
        ...
        <PrimaryButton title="Aage badho" onPress={() => setStage('geminiSetup')} />
        <Text style={styles.privacyCopy}>Permissions baad mein phone Settings se badli ja sakti hain.</Text>
      </ScrollView>
    );
  }

  if (stage === 'geminiSetup') {
    return (
      <ScrollView style={styles.screen} contentContainerStyle={{ paddingTop: insets.top + 24, paddingBottom: insets.bottom + 28 }} showsVerticalScrollIndicator={false}>
        <View style={styles.brandRow}><Pressable onPress={() => setStage('permissions')}><Feather name="arrow-left" size={22} color={colors.foreground} /></Pressable><Text style={styles.pageTitle}>Gemini brain</Text><Text style={styles.step}>2 / 2</Text></View>
        <View style={styles.geminiBadge}><Feather name="zap" size={18} color={colors.primary} /><Text style={styles.geminiBadgeText}>ONE LAST STEP</Text></View>
        <Text style={styles.sectionTitle}>Apni Gemini API key daalo</Text>
        <Text style={styles.sectionBody}>Ye key smart, general sawaalon ke jawab ke liye use hoti hai — bina key ke bhi Kavya time, timer, alarm, Maps, YouTube, calls jaise commands kaam karegi.</Text>
        ...
        {savedKeys.length > 0 && <Text style={styles.privacyCopy}>{savedKeys.length} key(s) saved hain. Settings mein jaake inhe kabhi bhi manage kar sakte ho.</Text>}
        <PrimaryButton title={savedKeys.length ? 'Kavya start karo' : 'Skip karo aur start karo'} onPress={() => saveStage('assistant')} />
      </ScrollView>
    );
  }
  ...
  if (stage === 'settings') {
    return (
      <ScrollView style={styles.screen} contentContainerStyle={{ paddingTop: insets.top + 24, paddingBottom: insets.bottom + 28 }} showsVerticalScrollIndicator={false}>
        ...
        {savedKeys.length === 0 ? (
          <View style={styles.emptyKeyCard}><Feather name="key" size={18} color={colors.mutedForeground} /><Text style={styles.keyHintText}>Abhi koi Gemini key saved nahi hai. Kavya default key ya local commands se kaam karegi.</Text></View>
        ) : savedKeys.map((meta) => (
  ...
  return (
    <View style={[styles.screen, { paddingTop: insets.top + 20 }]}>
      ...
      <View style={styles.capabilityCard}><View><Text style={styles.capabilityEyebrow}>WAKE PHRASE</Text><Text style={styles.capabilityText}>“Hey Kavya” · “Hi Kavya” · “Hello Kavya”</Text><Text style={styles.capabilityText}>Phir bolo: “Kavya tum kya kar sakti ho?”</Text></View><Feather name="message-circle" size={25} color={colors.primary} /></View>
