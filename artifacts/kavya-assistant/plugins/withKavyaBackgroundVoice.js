...
class KavyaVoiceService : Service(), RecognitionListener {
  ...
  override fun onCreate() {
    ...
    startRecognizer()
    muteRecognizerSounds(true)
  }

  private fun muteRecognizerSounds(mute: Boolean) {
    try {
  ...
  private fun restartRecognizer() {
    android.os.Handler(mainLooper).postDelayed({ if (!isServiceDestroyed) startRecognizer() }, 1200)
  }
  ...
  override fun onDestroy() { isServiceDestroyed = true; muteRecognizerSounds(false); recognizer?.cancel(); recognizer?.destroy(); recognizer = null; super.onDestroy() }
